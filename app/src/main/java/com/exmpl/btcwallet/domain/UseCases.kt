...
import com.exmpl.btcwallet.data.IllegalWalletState
import com.exmpl.btcwallet.data.Result
import com.exmpl.btcwallet.data.TransactionViewInfo
import com.exmpl.btcwallet.data.WTransaction
import com.exmpl.btcwallet.data.Wallet
import com.exmpl.btcwallet.data.netParams
import com.exmpl.btcwallet.data.toTransactionViewInfo
...
class UseCases @Inject constructor(private val wallet: Wallet): IUseCases {
    ...
    override fun sendMany(amount: String, address: String): Flow<Result> {
        ...
        return flow {
            ...
            wallet.sendTransaction(tx)
                ...
                .collect { txId = it }
            if (txId.isNotEmpty()) {
                emitAll(emitWNop(Result.SUCCESS(txId, tran.getFee())))
            } else {
    ...
    private fun parseAddress(address: String): Address? =
        try {
            ...
            Log.d(LOG_TAG, "Can't parse address: $address")
            null
        }
    ...
    private fun parseAmount(amount: String): Coin? =
        try {
            ...
            Log.d(LOG_TAG, "Can't parse amount: $amount")
            null
        }
